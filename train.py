...
from trainer import trainer_yolo, DEVICE
from icecream import ic



parser = argparse.ArgumentParser()
parser.add_argument('--dataset_path', type=str, help='filepath to YOLO dataset yaml')
parser.add_argument('--output', type=str, default='/output/sam/results')
parser.add_argument('--dataset', type=str,
                    default='YOLO', help='experiment_name')
...
parser.add_argument('--yolo_cfg', type=str, default='default_cfg.yaml',
                    help='Path to YOLO config to control data augmentations, do not change any '
                         'parameters in default config except augmentations')
...
if __name__ == "__main__":
    ...
    net = pkg.LoRA_Sam(sam, args.rank).to(DEVICE)  # .cuda()
    ...
    trainer = {'YOLO': trainer_yolo}

    with open(args.yolo_cfg, "r") as f:
        yolo_cfg = IterableSimpleNamespace(**yaml.safe_load(f))

    trainer[dataset_name](args, yolo_cfg, net, snapshot_path, multimask_output, low_res)
